...
const UNIVERSES = [
  ...
  { id: 'ai_chips', label: 'AI Chips' },
  { id: 'ai_data', label: 'AI Data Centers' },
  { id: 'ai_software', label: 'AI Software' },
  { id: 'moat', label: 'Huge Moat' },
  { id: 'compounders', label: 'Compounders' },
...
export default function Screener() {
  ...
  useEffect(() => { setPage(0); }, [sort, order, sector, debouncedQ, universe]);
  ...
  const fetchData = useCallback(() => {
    ...
    fetch(`/api/research/screener?${params}`, { signal: ctrl.signal })
      .then(async (r) => {
        const payload = await r.json() as ScreenerResponse;
    ...
    return () => ctrl.abort();
  }, [sort, order, sector, debouncedQ, page, universe]);
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-4 mb-3">
              ...
              <div className="flex bg-[#0d0d14] rounded border border-border p-0.5">
                {UNIVERSES.map((u) => (
                  <button
                    key={u.id}
                    onClick={() => { setUniverse(u.id); setSector(''); }}
                    className={cn(
                      "px-3 py-1 text-[10px] font-mono font-bold uppercase tracking-widest rounded transition-colors",
                      universe === u.id 
            ...
            <p className="text-[11px] text-muted-foreground font-mono uppercase tracking-widest mt-0.5">
              {data ? (
                <>
                  ...
                  {['sp500', 'qqq', 'dow30', 'space', 'ai_chips', 'ai_data', 'ai_software', 'moat', 'compounders'].includes(data.universe) ? ' · CURATED MEMBERS' : ' · THEMATIC / ALL ACTIVE'}
            ...
            <Card className="rounded-none border-border bg-muted/5 p-10 flex flex-col items-center gap-4 text-center">
              ...
              <div>
                ...
                <p className="text-muted-foreground font-mono text-xs mt-2 leading-relaxed max-w-md">
                  Run the seed script to populate universe constituents:
                </p>
