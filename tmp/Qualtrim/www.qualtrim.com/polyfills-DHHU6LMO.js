...
function Tt() {
    ...
    class t {
        ...
        static assertZonePatched() {
            if (ce.Promise !== O.ZoneAwarePromise) throw new Error("Zone.js has detected that ZoneAwarePromise `(window|global).Promise` has been overwritten.\nMost likely cause is that a Promise polyfill has been loaded after Zone.js (Polyfilling Promise api is not necessary when zone.js is loaded. If you must load one, do so before loading zone.js.)")
        }
        static get root() {
            let s = t.current;
        ...
        runTask(s, i, r) {
            if (s.zone != this) throw new Error("A task can only be run in the zone of creation! (Creation: " + (s.zone || K).name + "; Execution: " + this.name + ")");
            let E = s,
                {
                    type: B,
                    data: {
            ...
            if (s.state === q && (B === z || B === p)) return;
            let he = s.state != Z;
            he && E._transitionTo(Z, d);
            let _e = S;
            S = E, b = {
                parent: b,
                zone: this
            ...
            try {
                B == p && s.data && !ee && !A && (s.cancelFn = void 0);
                try {
                ...
                let Q = s.state;
                if (Q !== q && Q !== $)
                    if (B == z || ee || A && Q === y) he && E._transitionTo(d, Z, y);
                    else {
                        let Te = E._zoneDelegates;
                        this._updateTaskCount(E, -1), he && E._transitionTo(q, Z, q), A && (E._zoneDelegates = Te)
                    }
                b = b.parent, S = _e
            }
        }
        scheduleTask(s) {
            ...
            try {
                ...
                throw s._transitionTo($, y, q), this._zoneDelegate.handleError(this, r), r
            }
            return s._zoneDelegates === i && this._updateTaskCount(s, 1), s.state == y && s._transitionTo(d, y), s
        }
        scheduleMicroTask(s, i, r, E) {
            return this.scheduleTask(new g(U, s, i, r, E, void 0))
        }
        scheduleMacroTask(s, i, r, E, B) {
            return this.scheduleTask(new g(p, s, i, r, E, B))
        }
        scheduleEventTask(s, i, r, E, B) {
            return this.scheduleTask(new g(z, s, i, r, E, B))
        }
        cancelTask(s) {
            if (s.zone != this) throw new Error("A task can only be cancelled in the zone of creation! (Creation: " + (s.zone || K).name + "; Execution: " + this.name + ")");
            if (!(s.state !== d && s.state !== Z)) {
                s._transitionTo(x, d, Z);
                try {
                    this._zoneDelegate.cancelTask(this, s)
                } catch (i) {
                    throw s._transitionTo($, x), this._zoneDelegate.handleError(this, i), i
                }
    ...
    let c = {
        name: "",
        onHasTask: (M, s, i, r) => M.hasTask(i, r),
        onScheduleTask: (M, s, i, r) => M.scheduleTask(i, r),
        onInvokeTask: (M, s, i, r, E, B) => M.invokeTask(i, r, E, B),
        onCancelTask: (M, s, i, r) => M.cancelTask(i, r)
    };
    class f {
        get zone() {
            return this._zone
        ...
        invoke(s, i, r, E, B) {
            return this._invokeZS ? this._invokeZS.onInvoke(this._invokeDlgt, this._invokeCurrZone, s, i, r, E, B) : i.apply(r, E)
        }
        handleError(s, i) {
        ...
        _updateTaskCount(s, i) {
            let r = this._taskCounts,
                E = r[s],
                B = r[s] = E + i;
            if (B < 0) throw new Error("More tasks executed then were scheduled.");
            if (E == 0 || B == 0) {
                let ee = {
    ...
    class g {
        ...
        _zoneDelegates = null;
        _state = "notScheduled";
        constructor(s, i, r, E, B, ee) {
            if (this.type = s, this.source = i, this.data = E, this.scheduleFn = B, this.cancelFn = ee, !r) throw new Error("callback is not defined");
            this.callback = r;
            let A = this;
            s === z && E && E.useG ? this.invoke = g.invokeTask : this.invoke = function() {
                return g.invokeTask.call(ce, A, this, arguments)
            }
        }
    ...
    function H(M) {
        if (L || ce[k] && (L = ce[k].resolve(0)), L) {
            let s = L[P];
            s || (s = L.then), s.call(L, M)
        } else ce[T](M, 0)
    }

    function V(M) {
        J === 0 && _.length === 0 && H(Y), M && _.push(M)
    }
...
function Ge(e, n) {
    for (let a = e.length - 1; a >= 0; a--) typeof e[a] == "function" && (e[a] = Be(e[a], n + "_" + a));
    return e
}
...
function kt(e, n) {
    ...
    for (let t = 0; t < n.length; t++) {
        ...
        if (f) {
            ...
            e[c] = (T => {
                let k = function() {
                    return T.apply(this, Ge(arguments, a + "." + c))
                };
                return fe(k, T), k
            })(f)
        }
...
var nt = typeof WorkerGlobalScope < "u" && self instanceof WorkerGlobalScope,
    Se = !("nw" in X) && typeof X.process < "u" && X.process.toString() === "[object process]",
    Fe = !Se && !nt && !!(Pe && be.HTMLElement),
    rt = typeof X.process < "u" && X.process.toString() === "[object process]" && !nt && !!(Pe && be.HTMLElement),
    De = {},
    ...
    Xe = function(e) {
        ...
        let a = this || e.target || X,
            t = a[n],
            c;
        if (Fe && a === be && e.type === "error") {
            let f = e;
            c = t && t.call(this, f.message, f.filename, f.lineno, f.colno, f.error), c === !0 && e.preventDefault()
        } else c = t && t.apply(this, arguments), e.type === "beforeunload" && X[yt] && typeof c == "string" ? e.returnValue = c : c != null && !c && e.preventDefault();
        return c
    };
...
function Ye(e, n, a) {
    ...
    let f = t.get,
        ...
        T = n.slice(2),
        k = De[T];
    k || (k = De[T] = j("ON_PROPERTY" + T)), t.set = function(P) {
        let _ = this;
        if (!_ && e === X && (_ = X), !_) return;
        typeof _[k] == "function" && _.removeEventListener(T, Xe), g ? .call(_, null), _[k] = P, typeof P == "function" && _.addEventListener(T, Xe, !1)
    }, t.get = function() {
        let P = this;
        if (!P && e === X && (P = X), !P) return null;
        let _ = P[k];
        if (_) return _;
        if (f) {
            let R = f.call(this);
            if (R) return t.set.call(this, R), typeof P[mt] == "function" && P.removeAttribute(n), R
        }
        return null
    }, Ze(e, n, t), e[c] = !0
}
...
function ot(e, n, a) {
    if (n)
        for (let t = 0; t < n.length; t++) Ye(e, "on" + n[t], a);
    else {
        let t = [];
...
function ke(e) {
    let n = X[e];
    if (!n) return;
    X[j(e)] = n, X[e] = function() {
        let c = Ge(arguments, e);
        switch (c.length) {
            case 0:
                this[oe] = new n;
                break;
    ...
    let a = new n(function() {}),
        t;
    for (t in a) e === "XMLHttpRequest" && t === "responseBlob" || (function(c) {
        typeof a[c] == "function" ? X[e].prototype[c] = function() {
            return this[oe][c].apply(this[oe], arguments)
        } : Ze(X[e].prototype, c, {
            set: function(f) {
                typeof f == "function" ? (this[oe][c] = Be(f, e + "." + c), fe(this[oe][c], f)) : this[oe][c] = f
            },
            get: function() {
                return this[oe][c]
            }
    ...
    for (t in n) t !== "prototype" && n.hasOwnProperty(t) && (X[e][t] = n[t])
}
...
function vt(e, n, a) {
    ...
    function c(f) {
        ...
        }, t.apply(g.target, g.args), f
    }
    t = ue(e, n, f => function(g, T) {
        let k = a(g, T);
        return k.cbIdx >= 0 && typeof T[k.cbIdx] == "function" ? Ue(k.name, T[k.cbIdx], k, c) : f.apply(g, T)
    })
}
...
function Ct(e, n, a, t) {
    let c = t && t.add || He,
        f = t && t.rm || xe,
        g = t && t.listeners || "eventListeners",
        ...
        L = function(y, d, Z) {
            ...
            let x = y.callback;
            typeof x == "object" && x.handleEvent && (y.callback = p => x.handleEvent(p), y.originalDelegate = x);
            let $;
            try {
                y.invoke(y, d, [Z])
            } catch (p) {
            ...
            if (U && typeof U == "object" && U.once) {
                let p = y.originalDelegate ? y.originalDelegate : y.callback;
                d[f].call(d, Z.type, p, U)
            }
            return $
        };
    ...
    function H(y, d, Z) {
        ...
        if ($) {
            ...
            if ($.length === 1) {
                ...
                let p = $.slice();
                for (let z = 0; z < p.length && !(d && d[ct] === !0); z++) {
                    let O = L(p[z], x, d);
                    O && U.push(O)
                }
            }
    ...
    function K(y, d) {
        ...
        let p = y;
        for (; p && !p.hasOwnProperty(c);) p = je(p);
        if (!p && y[c] && (p = y), !p || p[k]) return !1;
        let z = d && d.eventNameToString,
            ...
            J = p[j(T)] = p[T],
            W;
        d && d.prepend && (W = p[j(d.prepend)] = p[d.prepend]);

        function M(o, u) {
            return u ? typeof o == "boolean" ? {
                capture: o,
                passive: !0
            } : o ? typeof o == "object" && o.passive !== !1 ? { ...o,
                passive: !0
            } : o : {
                passive: !0
            } : o
        ...
        let s = function(o) {
                if (!O.isExisting) return C.call(O.target, O.eventName, O.capture ? Y : V, O.options)
            },
            i = function(o) {
                if (!o.isRemoved) {
                    ...
                    if (D) {
                        for (let m = 0; m < D.length; m++)
                            if (D[m] === o) {
                                D.splice(m, 1), o.isRemoved = !0, o.removeAbortListener && (o.removeAbortListener(), o.removeAbortListener = null), D.length === 0 && (o.allRemoved = !0, o.target[v] = null);
                                break
                            }
                    }
                }
                if (o.allRemoved) return b.call(o.target, o.eventName, o.capture ? Y : V, o.options)
            },
            r = function(o) {
                return C.call(O.target, O.eventName, o.invoke, O.options)
            },
            E = function(o) {
        ...
        let l = function(o, u, v, D, m = !1, w = !1) {
            return function() {
                let I = this || e,
                    N = arguments[0];
                ...
                if (Se && N === "uncaughtException") return o.apply(this, arguments);
                let F = !1;
                if (typeof G != "function") {
                    if (!G.handleEvent) return o.apply(this, arguments);
                    F = !0
                }
                if (x && !x(o, G, I, arguments)) return;
                let de = !!Te && Te.indexOf(N) !== -1,
                    se = h(M(arguments[2], de)),
                    Ee = se ? .signal;
                if (Ee ? .aborted) return;
                if (Q) {
                    for (let ie = 0; ie < Q.length; ie++)
                        if (N === Q[ie]) return de ? o.call(I, N, G, se) : o.apply(this, arguments)
                }
                ...
                if (ge) {
                    if (We = !0, $) {
                        for (let ie = 0; ie < ge.length; ie++)
                            if (_e(ge[ie], G)) return
                    }
                } else ge = I[ze] = [];
                let Re, qe = I.constructor.name,
                    $e = st[qe];
                $e && (Re = $e[N]), Re || (Re = qe + u + (z ? z(N) : N)), O.options = se, Ve && (O.options.once = !1), O.target = I, O.capture = we, O.eventName = N, O.isExisting = We;
                let me = Z ? Rt : void 0;
                me && (me.taskData = O), Ee && (O.options.signal = void 0);
                let re = _t.scheduleEventTask(Re, G, me, v, D);
                if (Ee) {
                    O.options.signal = Ee;
                ...
                if (O.target = null, me && (me.taskData = null), Ve && (O.options.once = !0), typeof re.options != "boolean" && (re.options = se), re.target = I, re.capture = we, re.eventName = N, F && (re.originalDelegate = G), w ? ge.unshift(re) : ge.push(re), m) return I
            }
        };
        return p[c] = l(C, P, ee, A, U), W && (p[_] = l(W, R, E, A, U, !0)), p[f] = function() {
            let o = this || e,
                u = arguments[0];
            d && d.transferEventName && (u = d.transferEventName(u));
            let v = arguments[2],
                D = v ? typeof v == "boolean" ? !0 : v.capture : !1,
                m = arguments[1];
            if (!m) return b.apply(this, arguments);
            if (x && !x(b, m, o, arguments)) return;
            let w = ne[u],
                I;
            w && (I = w[D ? ae : le]);
            let N = I && o[I];
            if (N)
                for (let G = 0; G < N.length; G++) {
                    let F = N[G];
                    if (_e(F, m)) {
                        if (N.splice(G, 1), F.isRemoved = !0, N.length === 0 && (F.allRemoved = !0, o[I] = null, !D && typeof u == "string")) {
            ...
            for (let m = 0; m < D.length; m++) {
                let w = D[m],
                    I = w.originalDelegate ? w.originalDelegate : w.callback;
                v.push(I)
            }
            return v
        }, p[T] = function() {
            ...
            if (u) {
                ...
                if (v) {
                    ...
                    if (w) {
                        ...
                        for (let G = 0; G < N.length; G++) {
                            ...
                            this[f].call(this, u, de, F.options)
                        }
                    }
                    if (I) {
                        let N = I.slice();
                ...
                for (let D = 0; D < v.length; D++) {
                    let m = v[D],
                        w = it.exec(m),
                        I = w && w[1];
                    I && I !== "removeListener" && this[T].call(this, I)
                }
                this[T].call(this, "removeListener")
            }
            if (U) return this
        }, fe(p[c], C), fe(p[f], b), J && fe(p[T], J), S && fe(p[g], S), !0
    }
    let q = [];
    for (let y = 0; y < a.length; y++) q[y] = K(a[y], t);
    return q
}
    ...
    if (!n) {
        ...
        for (let g in e) {
            ...
            if (k && (!n || k === n)) {
                let P = e[g];
                if (P)
                    for (let _ = 0; _ < P.length; _++) f.push(P[_])
            }
        }
        return f
    }
...
function pe(e, n, a, t) {
    ...
    function T(P) {
        ...
        return Qe(R) ? _.handleId = R : (_.handle = R, _.isRefreshable = Je(R.refresh)), P
    }
    ...
    c = ue(e, n, P => function(_, R) {
        if (Je(R[0])) {
            let L = {
                    isRefreshable: !1,
                    isPeriodic: t === "Interval",
                    delay: t === "Timeout" || t === "Interval" ? R[1] || 0 : void 0,
                    args: R
                },
                H = R[0];
            R[0] = function() {
                try {
                    ...
                    !$ && !U && (x ? delete g[x] : Z && (Z[Ce] = null))
                }
            };
            let V = Ue(n, R[0], L, T, k);
            if (!V) return V;
            let {
                handleId: Y,
                handle: K,
            ...
            if (Y) g[Y] = V;
                ...
                K.refresh = function() {
                    ...
                    return x === "notScheduled" ? (V._state = "scheduled", Z._updateTaskCount(V, 1)) : x === "running" && (V._state = "scheduling"), d.call(this)
                }
            }
            return K ? ? Y ? ? V
        } else return P.apply(e, R)
    }), f = ue(e, a, P => function(_, R) {
        let L = R[0],
            H;
        Qe(L) ? (H = g[L], delete g[L]) : (H = L ? .[Ce], H ? L[Ce] = null : H = L), H ? .type ? H.cancelFn && H.zone.cancelTask(H) : P.apply(e, R)
    })
}
...
function It(e, n) {
    ...
    for (let c = 0; c < t.length; c++) {
        let f = n[t[c]];
        f ? .prototype && et(f.prototype, Ae(f.prototype), a)
    }
}
...
function Nt(e) {
    e.__load_patch("legacy", n => {
        ...
        let t = "clear";
        pe(n, "set", t, "Timeout"), pe(n, "set", t, "Interval"), pe(n, "set", t, "Immediate")
    }), e.__load_patch("requestAnimationFrame", n => {
        pe(n, "request", "cancel", "AnimationFrame"), pe(n, "mozRequest", "mozCancel", "AnimationFrame"), pe(n, "webkitRequest", "webkitCancel", "AnimationFrame")
    }), e.__load_patch("blocking", (n, a) => {
        let t = ["alert", "prompt", "confirm"];
        for (let c = 0; c < t.length; c++) {
            let f = t[c];
            ue(n, f, (g, T, k) => function(P, _) {
                return a.current.run(g, n, _, k)
            })
        }
        ...
        function P(_) {
            ...
            let V = L[Ne],
                Y = L[Le];
            if (!V) {
                let C = _.XMLHttpRequestEventTarget;
                if (C) {
                    let b = C.prototype;
                    V = b[Ne], Y = b[Le]
                }
            ...
            function y(C) {
                ...
                S[g] = !1, S[k] = !1;
                let J = S[f];
                V || (V = S[Ne], Y = S[Le]), J && Y.call(S, K, J);
                let W = S[f] = () => {
                    if (S.readyState === S.DONE)
                        if (!b.aborted && S[g] && C.state === q) {
                            let s = S[a.__symbol__("loadfalse")];
                            if (S.status !== 0 && s && s.length > 0) {
                                let i = C.invoke;
                                C.invoke = function() {
                                    let r = S[a.__symbol__("loadfalse")];
                                    for (let E = 0; E < r.length; E++) r[E] === C && r.splice(E, 1);
                                    !b.aborted && C.state === q && i.call(C)
                                }, s.push(C)
                            } else C.invoke()
                        } else !b.aborted && S[g] === !1 && (S[k] = !0)
                };
                return V.call(S, K, W), S[t] || (S[t] = C), z.apply(S, b.args), S[g] = !0, C
            }
            ...
            let x = ue(L, "open", () => function(C, b) {
                    return C[c] = b[2] == !1, C[T] = b[1], x.apply(C, b)
                }),
                $ = "XMLHttpRequest.send",
                U = j("fetchTaskAborting"),
                p = j("fetchTaskScheduling"),
                z = ue(L, "send", () => function(C, b) {
                    if (a.current[p] === !0 || C[c]) return z.apply(C, b); {
                        let S = {
                                target: C,
                                url: C[T],
                                isPeriodic: !1,
                            ...
                            J = Ue($, d, S, y, Z);
                        C && C[k] === !0 && !S.aborted && J.state === q && J.invoke()
                    }
                }),
                O = ue(L, "abort", () => function(C, b) {
                    let S = H(C);
                    if (S && typeof S.type == "string") {
                        if (S.cancelFn == null || S.data && S.data.aborted) return;
                        S.zone.cancelTask(S)
                    } else if (a.current[U] === !0) return O.apply(C, b)
                })
        }
    }), e.__load_patch("geolocation", n => {
        n.navigator && n.navigator.geolocation && kt(n.navigator.geolocation, ["getCurrentPosition", "watchPosition"])
    }), e.__load_patch("PromiseRejectionEvent", (n, a) => {
        function t(c) {
            return function(f) {
                lt(n, c).forEach(T => {
                    let k = n.PromiseRejectionEvent;
                    if (k) {
                        let P = new k(c, {
                            promise: f.promise,
                            reason: f.rejection
...
function Lt(e) {
    e.__load_patch("ZoneAwarePromise", (n, a, t) => {
        ...
        let T = t.symbol,
            k = [],
            P = n[T("DISABLE_WRAPPING_UNCAUGHT_PROMISE_REJECTION")] !== !1,
            _ = T("Promise"),
            R = T("then"),
            L = "__creationTrace__";
        ...
        function M(h, l, o) {
            ...
            if (h[y] === p) {
                ...
                if (l !== O && o instanceof A && o.hasOwnProperty(y) && o.hasOwnProperty(d) && o[y] !== p) i(o), M(h, o[y], o[d]);
                else if (l !== O && typeof v == "function") try {
                    ...
                    h[y] = l;
                    let D = h[d];
                    if (h[d] = o, h[Z] === Z && l === z && (h[y] = h[$], h[d] = h[x]), l === O && o instanceof Error) {
                    ...
                    for (let m = 0; m < D.length;) r(h, D[m++], D[m++], D[m++], D[m++]);
                    if (D.length == 0 && l == O) {
                        h[y] = C;
                        let m = o;
                        try {
...
` + o.stack : ""))
                        ...
                        P && (m.throwOriginal = !0), m.rejection = o, m.promise = h, m.zone = a.current, m.task = a.currentTask, k.push(m), t.scheduleMicroTask()
                    }
                }
            }
            return h
        ...
        function r(h, l, o, u, v) {
            i(h);
            let D = h[y],
                m = D ? typeof u == "function" ? u : K : typeof v == "function" ? v : q;
            l.scheduleMicroTask(U, () => {
                try {
                    let w = h[d],
                        I = !!o && Z === o[Z];
                    I && (o[x] = w, o[$] = D);
                    let N = l.run(m, void 0, I && m !== q && m !== K ? [] : [w]);
                    M(o, !0, N)
                } catch (w) {
                    M(o, !1, w)
                }
        ...
        class A {
            ...
            static resolve(l) {
                return l instanceof A ? l : M(new this(null), z, l)
            }
            static reject(l) {
                return M(new this(null), O, l)
            }
            static withResolvers() {
                let l = {};
                return l.promise = new A((o, u) => {
            ...
            static any(l) {
                ...
                let o = [],
                    u = 0;
                try {
                    for (let m of l) u++, o.push(A.resolve(m))
                } catch {
                ...
                let v = !1,
                    D = [];
                return new A((m, w) => {
                    for (let I = 0; I < o.length; I++) o[I].then(N => {
                        v || (v = !0, m(N))
                    }, N => {
                        D.push(N), u--, u === 0 && (v = !0, w(new ee(D, "All promises were rejected")))
                    })
                })
            }
            static race(l) {
                let o, u, v = new this((w, I) => {
                    o = w, u = I
                });
                ...
                for (let w of l) Y(w) || (w = this.resolve(w)), w.then(D, m);
                return v
            }
            static all(l) {
            ...
            static allWithCallback(l, o) {
                let u, v, D = new this((N, G) => {
                    ...
                    I = [];
                for (let N of l) {
                    Y(N) || (N = this.resolve(N));
                    let G = w;
                    try {
                        N.then(F => {
                            I[G] = o ? o.thenCallback(F) : F, m--, m === 0 && u(I)
                        }, F => {
                            o ? (I[G] = o.errorCallback(F), m--, m === 0 && u(I)) : v(F)
                        })
                    } catch (F) {
                        v(F)
                    }
                ...
                return m -= 2, m === 0 && u(I), D
            }
            constructor(l) {
                let o = this;
                if (!(o instanceof A)) throw new Error("Must be an instanceof Promise.");
                o[y] = p, o[d] = [];
                try {
                    let u = S();
                    l && l(u(b(o, z)), u(b(o, O)))
            ...
            then(l, o) {
                ...
                let v = new u(B),
                    D = a.current;
                return this[y] == p ? this[d].push(D, v, l, o) : r(this, D, v, l, o), v
            } catch (l) {
                ...
                u[Z] = Z;
                let v = a.current;
                return this[y] == p ? this[d].push(v, u, l, l) : r(this, v, u, l, l), u
            }
        }
        ...
        function Q(h) {
            ...
            let u = l.then;
            l[R] = u, h.prototype.then = function(v, D) {
                return new A((w, I) => {
                    u.call(this, w, I)
                }).then(v, D)
            }, h[_e] = !0
        ...
        return he && (Q(he), ue(n, "fetch", h => Te(h))), Promise[a.__symbol__("uncaughtPromiseErrors")] = k, A
    })
}
...
function Mt(e) {
    e.__load_patch("toString", n => {
        ...
        g[t] = a, Function.prototype.toString = g;
        let T = Object.prototype.toString,
            k = "[object Promise]";
        Object.prototype.toString = function() {
            return typeof Promise == "function" && this instanceof Promise ? k : T.call(this)
        }
    })
}

function At(e, n, a, t, c) {
    let f = Zone.__symbol__(t);
    if (n[f]) return;
    let g = n[f] = n[t];
    n[t] = function(T, k, P) {
        return k && k.prototype && c.forEach(function(_) {
            let R = `${a}.${t}::` + _,
                L = k.prototype;
            try {
                if (L.hasOwnProperty(_)) {
                    let H = e.ObjectGetOwnPropertyDescriptor(L, _);
                    H && H.value ? (H.value = e.wrapWithCurrentZone(H.value, R), e._redefineProperty(k.prototype, _, H)) : L[_] && (L[_] = e.wrapWithCurrentZone(L[_], R))
                } else L[_] && (L[_] = e.wrapWithCurrentZone(L[_], R))
            } catch {}
        }), g.call(n, T, k, P)
    }, e.attachOriginToPatched(n[t], g)
}

function Zt(e) {
    e.__load_patch("util", (n, a, t) => {
        let c = Ae(n);
        t.patchOnProperties = ot, t.patchMethod = ue, t.bindArguments = Ge, t.patchMacroTask = vt;
        let f = a.__symbol__("BLACK_LISTED_EVENTS"),
            g = a.__symbol__("UNPATCHED_EVENTS");
        n[g] && (n[f] = n[g]), n[f] && (a[f] = a[g] = n[f]), t.patchEventPrototype = Dt, t.patchEventTarget = Ct, t.isIEOrEdge = bt, t.ObjectDefineProperty = Ze, t.ObjectGetOwnPropertyDescriptor = ye, t.ObjectCreate = gt, t.ArraySlice = pt, t.patchClass = ke, t.wrapWithCurrentZone = Be, t.filterProperties = ut, t.attachOriginToPatched = fe, t._redefineProperty = Object.defineProperty, t.patchCallbacks = At, t.getGlobalObjects = () => ({
            globalSources: st,
...
function Ht(e, n) {
    for (let a = 1, t = 1; a < e.length; a++, t++)
        if (n[t] === "\\") t++;
...
var Oe = function(e, ...n) {
        if (Oe.translate) {
            let t = Oe.translate(e, n);
            e = t[0], n = t[1]
        }
        let a = dt(e[0], e.raw[0]);
        for (let t = 1; t < e.length; t++) a += n[t - 1] + dt(e[t], e.raw[t]);
        return a
    },
    xt = ":";

function dt(e, n) {
    return n.charAt(0) === xt ? e.substring(Ht(e, n) + 1) : e
}
globalThis.$localize = Oe;